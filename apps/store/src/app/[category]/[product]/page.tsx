...
import { Star, Truck, BadgeCheck, Zap } from "lucide-react";
...
export default function ProductPage({ params }: { params: Promise<Props["params"]> }) {
  ...
  const handleBuyNow = () => {
    ...
    const alreadyInCart = cartItems.find((i) => i.id === product);

    if (!alreadyInCart) {
      addItem({
        id: product,
        nome: productNames[product] || product,
        preco: 50,
        quantidade: 1,
        categoria: category,
      });
    ...
    <>
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-12">
        <div className="text-sm sm:text-base text-neutral-500 dark:text-neutral-400 mb-4 flex flex-wrap gap-2">
          ...
          <Link href={`/${category}`} className="hover:underline text-neutral-600 dark:text-neutral-400 capitalize">{category}</Link>
          <span className="select-none">/</span>
          <span className="capitalize font-medium text-neutral-900 dark:text-white">{product}</span>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
              ...
              <div className="flex items-center justify-center bg-neutral-50 dark:bg-neutral-800 rounded-xl p-6 sm:p-8">
                <Image src={imageSrc} alt={product} width={350} height={350} className="object-contain" />
              </div>
              ...
              <div className="space-y-6">
                ...
                <p className="text-sm sm:text-base text-neutral-700 dark:text-neutral-300 leading-relaxed">
                  Torne-se <strong>{product.toUpperCase()}</strong> no servidor e desbloqueie poderes exclusivos. Entre no jogo com status, conquiste respeito e aproveite benefícios que só os VIPs têm acesso.
                </p>

                <p className="text-xl sm:text-2xl font-bold text-green-600">R$ 50,00</p>
            ...
            <table className="w-full text-sm sm:text-base border border-neutral-200 dark:border-neutral-800 rounded-xl overflow-hidden">
              <tbody>
                <tr className="border-b border-neutral-200 dark:border-neutral-800">
                  <td className="p-3 font-medium bg-neutral-50 dark:bg-neutral-800">Servidor</td>
                  <td className="p-3">jogar.terax.world</td>
                </tr>
                <tr className="border-b border-neutral-200 dark:border-neutral-800">
                  <td className="p-3 font-medium bg-neutral-50 dark:bg-neutral-800">Tipo</td>
                  <td className="p-3">VIP {product.toUpperCase()}</td>
                </tr>
                <tr>
                  <td className="p-3 font-medium bg-neutral-50 dark:bg-neutral-800">Duração</td>
